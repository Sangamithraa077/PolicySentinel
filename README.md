...
1. **Upload** a policy (PDF, DOCX, or plain text).
2. **PDFs are read automatically** — Gemini extracts every clause's obligation: who must do what,
   how strongly, under what conditions.
...
A **PDF** upload runs this pipeline synchronously, start to finish, in one request. DOCX and
plain-text uploads are stored and downloadable but stop there — see [Known limitations](#known-limitations).
...
| **Uploads** | `POST /uploads/policies` — stores the document; for `.pdf` only, also runs the full pipeline (extraction → segmentation → obligations → comparison → conflicts → recommendations → graph sync) |
...
- **DOCX/plain-text uploads skip the AI pipeline.** They're stored and downloadable; extraction, segmentation, and conflict detection only run for `.pdf`.
